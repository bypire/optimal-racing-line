...
\noindent\colorbox{rwthblue25}{%
  ...
  against analytical references. The point mass on a friction circle is a deliberate baseline.\vspace{3pt}}}

\vspace{1.0em}

\begin{abstract}
\noindent
Given a track and a tyre-grip limit, the fastest way
...
boundary, $s=L$, and the lap time becomes a definite integral to minimise. The vehicle model is a baseline by
...
$1.2\times10^{-6}$; and the converged lap time
matches the closed-form skidpad optimum $T=2\pi\sqrt{R/\mu g}$ to $-0.2\,\%$, with the residual
explained by the finite track width. Mesh
...
3.43\,s (8.1\,\%) faster. The maximum defect is about
$5\times10^{-11}$.

From minimising time under
...
\begin{figure}[h]
  ...
  speed profile (brake, apex, accelerate).}
  \label{fig:lap}
\end{figure}
...
corners pull more $g$ than slow ones (up to $4.7g$ at $310\,\mathrm{km/h}$).
...
($40.8\,$s to $27.7\,$s across the swept range).
...
\begin{itemize}
  ...
  \item SLSQP is a dense SQP and does not scale past $N\!\sim\!250$ nodes; for much finer meshes an
        interior-point solver (IPOPT) exploiting sparsity is the right tool.
\end{itemize}
